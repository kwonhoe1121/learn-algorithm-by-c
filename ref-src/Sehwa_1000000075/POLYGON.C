...
int package_wrap(int p[], int n)
    ...
    t = p[0]; p[0] = p[mindex*2]; p[mindex*2] = t;
    t = p[1]; p[1] = p[mindex*2+1]; p[mindex*2+1] = t;
    m++;
    ...
    t = p[2]; p[2] = p[mindex*2]; p[mindex*2] = t;
    t = p[3]; p[3] = p[mindex*2+1]; p[mindex*2+1] = t;
    m++;
	...
	t = p[i*2]; p[i*2] = p[mindex*2]; p[mindex*2] = t;
	t = p[i*2+1]; p[i*2+1] = p[mindex*2+1];	p[mindex*2+1] = t;
	}
    return m;
    }
...
int jarvis_march(int p[], int n)
	...
	if (p[mindex*2+1] - p[i*2-1] < 0) break;
	t = p[i*2]; p[i*2] = p[mindex*2]; p[mindex*2] = t;
	t = p[i*2+1]; p[i*2+1] = p[mindex*2+1]; p[mindex*2+1] = t;
	m++;
	}
...
int ccw(int ax, int ay, int bx, int by, int cx, int cy)
    {
    long l;
    l = (long)bx*cy - (long)ay*bx - (long)ax*cy
	- (long)by*cx + (long)ax*by + (long)ay*cx;
    if (l > 0) return 1;
...
int graham_scan(int p[], int n)
    ...
    t = p[mindex*2]; p[mindex*2] = p[0]; p[0] = t;
    t = p[mindex*2+1]; p[mindex*2+1] = p[1]; p[1] = t;
