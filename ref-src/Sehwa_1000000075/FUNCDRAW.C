...
void bisect(double x1,double x2,double y1, double y2, double *x0, double *y0)
	...
	x=(x1+x2)/2.0;
	y=(y1+y2)/2.0;
	if(f(x1,y1)*f(x,y)<=0.0) { x2 = x; y2 = y; }
	else { x1 = x; y1 = y; }
	}
    *x0 = x;
    *y0 = y;
...
double bisect_c(double x0,double y0,double c, double t)
	...
	{
	q=(q1+q2)/2.0;
	if(f_c(q1,x0,y0,c)*f_c(q,x0,y0,c)<=0.0) q2 = q;
	else q1 = q;
	}
    found_flag = 1;
    return q;
...
void d_2d(double x, double y, double q, double c, int flag,
	...
	qq = bisect_c(x0,y0,c,t);
        ...
        if (x0 < -320./k+x1 || x0 > 320./k+x1
                            || y0 < -240./k+y1 || y0 > 240./k+y1)
            {
            if (flag) d_2d(x,y,q+M_PI,c,0,t0,k,x1,y1);
            break;
            }
...
void main()
    ...
    line(320,0,320,480);
    line(0,240,640,240);
    for(i = 0; i < 1; i++) printf("%d",draw(example,1,0.01,100,0,0));
    getch();
    closegraph();
    }
