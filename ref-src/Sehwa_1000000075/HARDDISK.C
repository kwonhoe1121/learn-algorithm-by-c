...
typedef enum { seeking, wait_sector, idling } state;
...
state busy_HardDisk(void)
    {
    if (HardDisk.State == idling) return 0;
    else return 1;
    }
...
void oper_HardDisk()
    ...
    if (HardDisk.sector > 64) HardDisk.sector -= 64;
    if (HardDisk.State == seeking) 
	{
	HardDisk.rest_time--;
	HardDisk.track += (HardDisk.track > HardDisk.target_track) ? 
	...
	{
	if (HardDisk.target_sector < SECT_PER_MSEC 
	    && HardDisk.sector >= SECT_PER_MSEC)
	    {
...
void display(unsigned long t, int event)
    ...
    if (HardDisk.State == idling) cprintf("idling     ");
    else if (HardDisk.State == seeking) cprintf("seeking    ");
    else if (HardDisk.State == wait_sector) cprintf("wait_sector ");
    gotoxy(5, 12);
