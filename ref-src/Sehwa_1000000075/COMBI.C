...
unsigned long _combi_2(int n, int r)
    {
    if (r == 0 || r == n) return 1;
    if (!c[n-r-1]) c[n-r-1] = _combi_2(n-1, r-1);
...
unsigned long combi_3(int n, int r)
    ...
    if(r == 1) return n;
    if(r > 17) return 0;
    for (i = 1; i < r; i++) d[i] = i + 2;
    for (i = 3; i <= n-r+1; i++)
        {
        d[0] = i;
        for (j = 1; j < r; j++) d[j] += d[j-1];
        }
    return d[r-1];
    }
...
void knapsack()
    ...
    while (1)
        ...
        if (total_size > amount) continue;
        if (max_val < value)
            {
            max_size = total_size;
            max_val = value;
            for (i = 0; i <item_num; i++) max_array[i] = b[i];
            }
        }
    }
