...
int regress(int l, double t[][2], int n, double (*f[])(double), double c[])
    ...
    for (i = 0; i < n; i++)
        {
        v[i] = sumy(l,t,f[i]);
        for (j = 0; j < n; j++) m[i][j] = sumx(l,t,f[i],f[j]);
        }
    if(!invrs_matrix(m,invr,n)) return 0;
...
int regress_1(int l, double t[][2], double c[])
    ...
    for (i = 0; i < l; i++) sx += t[i][0];
    for (i = 0; i < l; i++) sy += t[i][1];
    for (i = 0; i < l; i++) sxx += t[i][0]*t[i][0];
    for (i = 0; i < l; i++) sxy += t[i][0]*t[i][1];
